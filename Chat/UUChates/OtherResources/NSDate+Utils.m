...
#pragma mark --通过秒数 得到时间显示
+(NSString *)getTimeShowByTimeSeconds:(NSString *)timeStr withChinese:(BOOL)isChinese
{
...
//    long long time = [timeStr longLongValue];
    ...
    if(timeInterval < D_HOUR){//1小时内{
        
        NSInteger min = timeInterval / D_MINUTE;
        
        if(min <1){
            
            long long timeInt = (long long)timeInterval ;
            
            NSInteger seconds = timeInt % D_HOUR;
            
            returnStr = [NSString stringWithFormat:@"%zi秒前",seconds];
            
        }else{
            
            returnStr = [NSString stringWithFormat:@"%zi分钟前",min];
            
        }
        
    }else if(timeInterval < D_DAY){//1天内
        
        NSInteger hour = (NSInteger)(timeInterval / D_HOUR);
        
        returnStr = [NSString stringWithFormat:@"%zi小时前",hour];
        
    }else if(timeInterval < D_DAY *30){ //一个月内
        
        if(isChinese){
            returnStr = [date stringMonthDayTimeChinese];
            
        }else{
            returnStr = [date stringMonthDayTime];
        }
        
    }else{
        NSInteger year = [date year];
        
        NSInteger nowYead = [now year];
        
        if(year == nowYead){ //如果是同一年
            ...
            returnStr = [date stringYearMonthDay];
        }
...
#pragma mark - 通过秒数 得到时间显示 详情页
+(NSString *)getTimeShowByTimeSeconds:(NSString *)timeStr{
    ...
    }else if(timeInterval < D_DAY *7){ //7天内
        
        NSInteger day = (NSInteger)(timeInterval / D_DAY);
        
        returnStr = [NSString stringWithFormat:@"%zi天前",day];
    }
...
{
    return [NSDate dateMonthDayTimeWithDate:self];
}

-(NSString *)stringMonthDayTimeChinese
{
    return [NSDate dateMonthDayTimeWithDateChinese:self];
}
...
+(NSString *)dateMonthDayTimeWithDate:(NSDate *)date
{
    if (date == nil) {
        date = [NSDate date];
    }
...
+(NSString *)dateMonthDayTimeWithDateChinese:(NSDate *)date
{
    if (date == nil) {
        date = [NSDate date];
    }
