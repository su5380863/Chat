...
#define CUSTOMCOLOR_ALPHA(R,G,B,A)      ([UIColor colorWithRed:(R)/255.0 green:(G)/255. blue:(B)/255.0 alpha:A])
...
-(id)init
{
    ...
    if(self){
        ...
        self.keyWorkColor=CUSTOMCOLOR_ALPHA(56, 56, 255, 0.7);
        self.iconSize=16.0f;
        self.line=5.0f;
        self.paragraph=5.0f;
...
-(void)match:(NSString*)source
{
    ...
    NSRegularExpression * regular=[[NSRegularExpression alloc]initWithPattern:MatchImage_Regular options:NSRegularExpressionDotMatchesLineSeparators|NSRegularExpressionCaseInsensitive error:nil];
    ...
    for(NSDictionary * dic in imageArr){
        ...
        NSDictionary* imgAttr = [NSDictionary dictionaryWithObjectsAndKeys: //2
                                 [NSNumber numberWithFloat:(self.iconSize)], @"width",
                                 nil] ;
...
-(void)buildFrames
{
    static CGFloat maxHeight = MatchMaxHeight; //10000
    CGMutablePathRef path = CGPathCreateMutable(); //2
    CGRect textFrame = CGRectMake(0,0, width, maxHeight);
    ...
    NSUInteger length = [self.attrString length];
...
#pragma -mark 获得内容的总高度
    ...
    if([lines count]>=1){
        ...
        float total_height =maxHeight- line_y + descent+1 ;    //+1为了纠正descent转换成int小数点后舍去的值
...
-(void)matchUrlLink:(NSString*)source attrString:(NSMutableAttributedString*)attrString1 offset:(NSInteger)offset link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    for( NSTextCheckingResult * result in array){
        if(link==nil){
            CTFontRef fontRef=CTFontCreateWithName((__bridge CFStringRef)(self.font.fontName),self.font.pointSize,NULL);
            NSDictionary *attribute=[NSDictionary dictionaryWithObjectsAndKeys:(__bridge id)fontRef,kCTFontAttributeName,(id)self.keyWorkColor.CGColor,kCTForegroundColorAttributeName,[NSNumber numberWithInteger:NSUnderlineStyleSingle],NSUnderlineStyleAttributeName,nil];
